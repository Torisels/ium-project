...
def build_app(users_service: UsersService, models_service: ModelsService, product_service: ProductsService,
    ...
    def recommend(model_id: str, user_id: int = 0):
    ...
    @app.get("/api/experiments/online/current/clear")
    def delete_log(experiment_id: int):
        experiments_service.clear_current_log()
        return {"deleted_log": experiment_id}
    ...
    async def register_event(event: EventRequest):
        experiments_service.log_event(event.user_id, event.product_id, event.event_type)
        return event
